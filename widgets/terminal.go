...
func (term *Terminal) Draw() {
	...
	for _, rect := range term.damage {
		for x := rect.StartCol(); x < rect.EndCol() && x < w; x += 1 {

			for y := rect.StartRow(); y < rect.EndRow() && y < h; y += 1 {
				...
				term.Print(x, y, style, string(cell.Chars()))
			}
		}
	}
